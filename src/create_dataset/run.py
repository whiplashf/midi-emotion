...
if __name__ == "__main__":
    ...
    if use_pianoroll_dataset:
        midi_dataset_path = "../data_files/lpd/lpd_full"
        extension = ".npz"
    ...
    if os.path.exists(output_path) and not redo:
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=2) as executor: # 设置最大线程数是2
            midi_file_to_hash = list(tqdm(executor.map(get_hash_and_file, file_paths), total=len(file_paths)))
        # midi_file_to_hash = get_hash_and_file(file_paths)
    ...
    if os.path.exists(output_path) and not redo:
        ...
        midi_files_matched_unique = sorted(list(set(midi_files_matched).intersection(midi_files_unique)))  # intersection取交集
        if write:
            with open(output_path, "w") as f:
